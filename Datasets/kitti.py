...
class KittiDataset:
    ...
    def load_inputs(self, idx: int) -> Dict: 
        img = Image.open(self.image_paths_left[idx])
        right_img = Image.open(self.image_paths_right[idx])
        inputs = {
            "image": img,
    ...
    def left_to_right(self) -> np.ndarray:
        k1, r1, t1, _, _, _, _ = cv2.decomposeProjectionMatrix(self.P_l)
        k2, r2, t2, _, _, _, _ = cv2.decomposeProjectionMatrix(self.P_r)
        return (t1 - t2).squeeze()
